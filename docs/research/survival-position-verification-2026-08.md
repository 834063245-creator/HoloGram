...
## 0. 现在到哪了（TL;DR，2026-08-15）

**工程部分：完工。** P0 全部 ✅（文件级依赖图 TS 上与 depcruise 打平、Python/Rust 达标、失败不静默）；P1-2（LSP 回写）✅；P1-4（增量 stale 治理）✅；P1-1（SCIP 桥接）✅——真实 scip-typescript 实测 gap_probe_ts：合并后 gold recall **11/11、0 误报、0 负数边**，SCIP +24 条编译器级 usage 边。

剩下三件，都不阻塞发布：

| 剩余 | 类型 | 一句话 |
|---|---|---|
| P1-1 基准扩围 | 工程收尾 | 引用边真值集扩围 + `scripts/bench_scip_bridge.py` 接 CI |
| P1-3 CI 门禁 | 工程收尾（停着） | 基准已 100%，按项目规则「不要改 CI」暂不动 `.github/workflows/ci.yml` |
| 产品验证 | 非工程 | 5 个真实用户在整体产品里下真实任务（改代码/审架构），观测 **Agent 对图工具的调用率**与任务完成质量——图是 Agent 的内部工具，人不负责问图 |

想快速了解：竞品依据看 `competitor-landscape-full-2026-08.md`（全部文档索引见 `research/README.md`）；SCIP 分档看 `scip-bridge-tiering-decision-2026-08.md`。
...
**现状（2026-08-15 更新）**：L1 ✅（27 语言管线真实工作）、L3 ✅（已建成）、**L2 ✅（工程侧）**——P0 全闭环、P1-2/P1-4 落地、SCIP 桥接已通并有首份精度对比。剩余收尾见 §0。
...
| **Agent 工作流不成立**（LLM 习惯直接 grep/读文件，图谱查询未成为刚需） | Agent 会话数据中图工具调用率持续低 | 图是给 Agent 用的，不是给人手动查的——**人不会「先问一句图」，这个行为的主体是 Agent**。可观测判据 = 每次真实任务里 Agent 对图工具的调用率、图调用后任务完成质量。产品验证的正确形态：用户在整体产品（桌面端 + Agent + 3D 星图）里下真实任务，观察 Agent 是否自主用图，而不是教用户问图 |
...
7. ⬜ **产品验证（非工程）**：图是给 Agent 用的内部工具——验证形态 = 5 个真实用户在整体产品里下真实任务，观测 Agent 图工具调用率与任务完成质量（不是教用户「改前问一句图」）。这一步的结果决定 P1 之后的投资节奏。

---

## 5. 一句话

工程底线已站住：P0 让文件级依赖图在 TS 上与 depcruise 打平、Python/Rust 达标、失败不静默；P1-2/P1-4 补齐信任闭环；P1-1 SCIP 桥接把主流语言的符号级引用从「同名猜测」升级为「编译器级解析」（实测 11/11、0 误报）。剩下的不是「有没有漏掉的关键工程项」，而是两件收尾小事（基准扩围、CI 门禁）加一件大事（产品验证：Agent 图查询是否成为刚需）。
