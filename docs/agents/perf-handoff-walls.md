...
## 开发工作流:Agent 平台环境现状(2026-08-06 实测)

**背景**:HoloGram 的开发借助 Agent 工具完成(本窗口即跑在 HanaAgent 平台上)。
用户对 Agent 平台有横向对比经验(Claude Code 等),以**基础工程体验**为先。

### HanaAgent 平台实测问题清单(2026-08-06)

1. **renderer 渲染进程持续空转**:两次 8s 采样均 ~40% 单核 CPU
   (gpu-process 另 ~25%),从会话启动即开始,疑似动画/渲染循环无停止条件;
   用户报告关闭软件可能不回收(未实测确认残留)。
2. **长任务无完成回传**:exec_command 一次性执行,30 分钟级任务完成后
   Agent 无感知,需用户提醒或 subagent 盯梢。
3. **tty transcript 吞输出**:tty 会话的引擎输出未记录进 transcript,
   长任务报告丢失(实测 kernel 终验输出丢失一次)。
4. **one-shot 命令空等超时**:cargo test 后命令空挂 900s(测试早已完成),
   多次发生,疑似管道/调度问题。
5. **进程感知盲区**:Agent 无法直接感知外部进程完成/状态,
   需 WorkingSet/文件时间戳/句柄数等间接推断。

### 工作流应对(已验证可行)

- 长任务 = `Start-Process` 独立进程 + stdout/stderr 落文件 + subagent 盯梢轮询回传
- 完成判定 = 文件停止增长 + 终态行(如 iter 行)落盘
- 进程健康 = CPU 时间增长 + 工作集变化 + 目标产物文件 mtime

### 决策背景

用户评估后认为 HanaAgent 当前状态**不适合作为主要开发工作流平台**(渲染空转
拖慢系统 + 基础 coding 流程问题),倾向找无此类 bug 的 Agent 工具推进 HoloGram
开发。HanaAgent 与 HoloGram 属竞品(用户与其作者同批报名崔添翼 DeepSeek
harness 内测)。后续接手窗口:优先用验证过的长任务方法论,平台若切换需
重新确认上述问题在目标平台上是否仍存在。
