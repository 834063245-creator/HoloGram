...
> P7g headless 实测勾销（2026-07-19，CDP 合成事件）：信标模式机四态 ✓ · 面板开合+rail 让位 ✓ · resize 夹取+切模式清残留 ✓ · 键盘 Ctrl+K/L/,/? 与 Esc 逐层链 ✓ · 氛围（grain 无摩尔纹）✓。仍留真机：权限卡/goal/星图 ⚠ 项、悬停、历史 active、碰撞、Welcome、消息流内容态。
...
## 四′、开工协议（给执行模型 —— 每期开工必读，别跳）

> 本节是防返工协议。执行模型能力假设：**会照做、不发挥**。凡是本文件没让你做的事，一律不做。
...
3. 样式只追加覆盖段到对应 CSS 末尾，不重排存量规则；只用 --obs-* 变量与配方值。
4. 不改任何 .ts/.tsx 逻辑（P7c/P7d 允许改内联样式值，P7g 除外——P7g 本来就该动逻辑）。
5. 完工标准 = 门禁三绿 + git diff --stat 自证范围 + cdp-shot 截图 + 状态表更新。
6. 遇到 plan 没写清的地方：停下问，不猜。
```

**完工自证清单（执行模型必须在汇报里逐项回答）**：

- [ ] `git diff --stat` 列出了哪些文件？是否全部在 P7x 一节的对象清单内？
- [ ] 门禁三条命令的实际输出结尾（贴原文）？
- [ ] 覆盖了哪些选择器/组件？哪些是配方里有的、哪些是自己发挥的？（发挥的必须标注理由）
- [ ] 截图验证了什么部位？什么没验证到（如实说）？
- [ ] handoff 与本文件状态表是否已更新？

**执行边界提醒（擦屁股高发点，逐条核对）**：

- 不许「顺手」修 biome 基线 288 个历史 error；不许重排/精简存量 CSS；不许重命名类名「为了更好读」。
- 不许给组件加新 prop/状态；不许「优化」chat-core/面板组件逻辑。
- 配方里有的用配方值，配方没有的用 §2 设计规范推，**不允许发明第三类色/第三种圆角**。
- 每期的「只改表面」原则覆盖一切：类名/id/DOM 结构/事件/状态机全是契约。
...
| 阶段 | 内容 | 状态 | Commit |
|---|---|---|---|
| **P7g** | **行为与动效修复（回归审计已坐实 6 项，最先做）** | ✅ | `36b1c19` `37be18a` `4dffede` `874f9b9` `d55f5c9`（动效 = WAAPI 单轨 + morphing 类 + rAF 等类翻牌；约束面板入 Esc 链） |
| P7a | 消息流内部 | ✅ | `e1a5fb7` |
| P7b | 六面板内部 | ✅ | `c56a4b7` |
| P7c | 浮层与菜单 | ✅ | `bc84362` |
| P7d | FileViewer/翻译器 | ✅ | `3f253cf` |
| P7e | chrome 细节 | ✅ | `1c3bbc8` |
| P7f | 一致性清扫 | ✅ | `e7567f4` |
