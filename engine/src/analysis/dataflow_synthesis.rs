...
pub fn synthesize_dataflow_edges(graph: &mut Graph, project_root: &Path, parse_cache: &ParseCache) -> usize {
    ...
    for node in graph.nodes.values() {
        if let Some(ref loc) = node.location {
            if let Some(p) = loc.rsplit_once(':').map(|(f, _)| f) {
                if p.len() > 1 { files.insert(p.replace('\\', "/")); }
            }
        }
    }
...
fn node_id_for(graph: &mut Graph, name: &str, file: &str, line: usize) -> String {
    for (id, node) in &graph.nodes {
        if node.name == name { return id.clone(); }
    }
...
fn insert_edge(g: &mut Graph, eid: &str, src: &str, tgt: &str, kind: EdgeKind, depth: u8, delay: Option<f64>) -> usize {
    if g.get_edge(eid).is_none() && src != tgt {
        g.add_edge(Edge{id:eid.into(),source:src.into(),target:tgt.into(),kind,coupling_depth:depth,cross_file:false,direction:"synthesized".into(),temporal_delay_sec:delay,medium_node_id:None});
        1
    } else { 0 }
}
