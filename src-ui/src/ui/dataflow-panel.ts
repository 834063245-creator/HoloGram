...
export class DataflowPanel {
  ...
  constructor(container: HTMLElement) {
    ...
    // Auto-refresh when Agent saves a new trace
    bus.on('dataflow:saved', () => {
  ...
  private renderTraceList(): void {
    ...
    this.left.style.cssText =
      'width:260px; min-width:180px; display:flex; flex-direction:column; border-right:1px solid rgba(40,70,130,0.15);';
    ...
    input.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' && !e.shiftKey) {
        e.preventDefault();
        this.doExplore(input.value.trim());
      }
    });
    ...
    return blocks
      .map((block) => {
        ...
        if (first.match(/^[-*]\s/)) {
          const items = lines
            .filter((l) => l.trim())
            .map((l) => `<li>${this.inlineMd(l.replace(/^[-*]\s*/, ''))}</li>`)
            .join('');
        ...
        if (first.match(/^\d+\.\s/)) {
          const items = lines
            .filter((l) => l.trim())
            .map((l) => `<li>${this.inlineMd(l.replace(/^\d+\.\s*/, ''))}</li>`)
            .join('');
  ...
  private renderEngineExplore(explore: any): string {
    ...
    if (Object.keys(relationships).length)
      parts.push(this.renderRelationships(relationships, Object.keys(relationships)));
    if (sourceCode.length) parts.push(this.renderSourceCode(sourceCode));
    if ((blastRadius.dependents || []).length || (blastRadius.tests || []).length)
      parts.push(this.renderBlastRadius(blastRadius.dependents || [], blastRadius.tests || []));
  ...
  private renderSourceCode(sources: any[]): string {
    const snippets = sources
      ...
      .map(
        (s: any) =>
          `<div class="df-src-item"><div class="df-src-loc">${this.esc(s.file || '')}${s.line ? ':' + s.line : ''}</div><pre class="df-src-code">${this.esc(s.code || '')}</pre></div>`,
      )
      .join('');
  ...
  private onDragStart(e: PointerEvent): void {
    ...
    this.dragStart = {
      x: e.clientX,
      y: e.clientY,
      elX: parseInt(this.el.style.left) || 0,
      elY: parseInt(this.el.style.top) || 0,
      w: 0,
      h: 0,
    };
