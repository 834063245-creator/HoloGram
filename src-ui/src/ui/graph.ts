...
// ── Pure procedural layout — zero force simulation ──────────
// 1. Community centroids: Fibonacci sphere + cross-edge attraction
// 2. Community internals: spiral arms, hubs at center
// O(n + C²) total, no GPU needed, no convergence concerns.

function proceduralLayout(
  n: number,
  nodeComm: number[],
  nodeDeg: number[],
  crossEdges: Array<{ s: number; t: number }>, // cross-community edges
  shellRadius: number,
): Float32Array {
  const pos = new Float32Array(n * 3);

  // ── Build communities ──
  type Comm = { id: number; cx: number; cy: number; cz: number; nodes: number[] };
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!cc) { cc = { id: c, cx: 0, cy: 0, cz: 0, nodes: [] }; commMap.set(c, cc); }
    cc.nodes.push(i);
  }
  const commArr = [...commMap.values()];

  // ── Build community adjacency (cross-edge counts) ──
  const commAdj = new Map<number, Map<number, number>>();
  for (const e of crossEdges) {
    const sc = nodeComm[e.s], tc = nodeComm[e.t];
    if (sc < 0 || tc < 0 || sc === tc) continue;
    if (!commAdj.has(sc)) commAdj.set(sc, new Map());
    if (!commAdj.has(tc)) commAdj.set(tc, new Map());
    commAdj.get(sc)!.set(tc, (commAdj.get(sc)!.get(tc) || 0) + 1);
    commAdj.get(tc)!.set(sc, (commAdj.get(tc)!.get(sc) || 0) + 1);
  }

  // ── Place centroids on Fibonacci sphere (sorted by size) ──
  commArr.sort((a, b) => b.nodes.length - a.nodes.length);
  for (let k = 0; k < commArr.length; k++) {
    const phi = Math.acos(1 - 2 * (k + 0.5) / commArr.length);
    const theta = Math.PI * (1 + Math.sqrt(5)) * k;
    commArr[k].cx = Math.cos(theta) * Math.sin(phi) * shellRadius;
    commArr[k].cy = Math.cos(phi) * shellRadius;
    commArr[k].cz = Math.sin(theta) * Math.sin(phi) * shellRadius;
  }

  // ── Attract connected centroids toward each other (few iterations) ──
  for (let iter = 0; iter < 8; iter++) {
    for (const ca of commArr) {
      const neighbors = commAdj.get(ca.id);
      if (!neighbors) continue;
      let fx = 0, fy = 0, fz = 0, totalW = 0;
      for (const [nbId, w] of neighbors) {
        const cb = commMap.get(nbId);
        if (!cb) continue;
      ...
      if (totalW > 0) {
        const s = shellRadius * 0.015 / totalW;
        ca.cx += fx * s; ca.cy += fy * s; ca.cz += fz * s;
      }
    }
  }

  // ── Spiral internals + assign to pos array ──
  ...
  for (let j = 0; j < unassigned.length; j++) {
    ...
    const r = shellRadius * 0.08 * Math.cbrt(j + 1);
