...
const FileTranslatorApp: React.FC<{
...
export class FileTranslatorController {
  private _visible = false;
  private _filePath: string | null = null;
  private _selectionText: string | null = null;
  ...
  constructor(
    ...
    this._getEditorContent = getEditorContent;

    // Container wraps divider + panel
  ...
  private async _render(): Promise<void> {
    ...
    if (!this._root) this._root = createRoot(this._panelEl);
    this._root.render(
      React.createElement(FileTranslatorApp, {
        key: this._filePath || Date.now(),
        filePath: this._filePath,
        onClose: () => this.destroy(),
        onLayoutChange: this._onLayoutChange,
        getEditorContent: this._selectionText
          ? () => this._selectionText
