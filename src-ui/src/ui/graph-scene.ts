...
// ── Observatory graticule (刻度环 + 黄道椭圆) ─────────────────
// P5：仪器刻度盘 —— prototype/observatory-concept.html drawGraticule 的 3D 版。
// 单位半径几何（半径 1），facade 按图半径统一 scale；顶点色预乘透明度 +
// 加法混合，在深空底色上模拟原型里的低透明度刻线。刻度盘是参考系，不随
// galaxyGroup 旋转，直接挂 scene。

export function buildGraticule(scene: THREE.Scene): THREE.Group {
  ...
  // 原型颜色预乘 alpha：rgba(150,170,215,·) / rgba(201,162,92,·)
  const premul = (r: number, g: number, b: number, a: number): [number, number, number] => [
    (r / 255) * a,
    (g / 255) * a,
    (b / 255) * a,
  ];
  const ringCol = premul(150, 170, 215, 0.28); // 同心环（原型 alpha 0.05，加法混合下需提亮）
  const spokeCol = premul(150, 170, 215, 0.2); // 径向线（原型 0.035）
  const tickCol = premul(170, 190, 225, 0.4); // 刻度（原型 0.08）
  const eclipticCol = premul(201, 162, 92, 0.42); // 黄道椭圆 · 黄铜签名（原型 0.07）

  const TAU = Math.PI * 2;
  const circle = (radius: number, y: number, col: [number, number, number], segments = 160) => {
  ...
  // 同心环 0.34 / 0.6 / 0.86
  for (const k of [0.34, 0.6, 0.86]) circle(k, 0, ringCol);
  // 径向线 ×12（0.12 → 0.86）
  for (let k = 0; k < 12; k++) {
    const a = (k / 12) * TAU;
    const c = Math.cos(a);
    const s = Math.sin(a);
    seg(c * 0.12, 0, s * 0.12, c * 0.86, 0, s * 0.86, spokeCol[0], spokeCol[1], spokeCol[2]);
  }
  // 刻度环 ×144（每 12 个一根主刻度）
  for (let k = 0; k < 144; k++) {
    const a = (k / 144) * TAU;
    const len = k % 12 === 0 ? 0.045 : 0.018;
    const c = Math.cos(a);
    const s = Math.sin(a);
    seg(c * (1 - len), 0, s * (1 - len), c, 0, s, tickCol[0], tickCol[1], tickCol[2]);
  }
  // 黄道椭圆 —— 半径 0.94 的圆绕 X 轴倾 23.4°（黄赤交角），相机俯视下呈椭圆
  const OBLIQUITY = 0.4084;
  const cosO = Math.cos(OBLIQUITY);
  const sinO = Math.sin(OBLIQUITY);
  const eclipticSegs = 192;
  for (let i = 0; i < eclipticSegs; i++) {
    const a0 = (i / eclipticSegs) * TAU;
    const a1 = ((i + 1) / eclipticSegs) * TAU;
    const [x0, z0] = [Math.cos(a0) * 0.94, Math.sin(a0) * 0.94];
    const [x1, z1] = [Math.cos(a1) * 0.94, Math.sin(a1) * 0.94];
    seg(x0, -z0 * sinO, z0 * cosO, x1, -z1 * sinO, z1 * cosO, eclipticCol[0], eclipticCol[1], eclipticCol[2]);
  }
