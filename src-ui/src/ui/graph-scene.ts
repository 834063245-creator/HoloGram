...
import { makeGlowPointMaterial, makeCoreFresnelMaterial } from './graph-shaders';
...
export function buildNebulaDust(scene: THREE.Scene, glowTex: THREE.Texture): { points: THREE.Points; phases: number[] } {
  const count = 300;
  ...
  const mat = new THREE.PointsMaterial({
    size: 18, map: glowTex, blending: THREE.AdditiveBlending,
...
export function buildGalaxyClouds(
  ...
  for (let gi = 0; gi < galaxyMeta.length; gi++) {
    ...
    commFoldGroup.add(halo); glows.push(halo);
    ...
    commFoldGroup.add(core); glows.push(core);
  }
  return { clouds, glows };
}
...
export function updateGalaxyLabels(
  ...
  for (let gi = 0; gi < divs.length; gi++) {
    ...
    if (v.z > 1) { divs[gi].style.display = 'none'; continue; }
